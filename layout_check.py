...
def _get_vio_zone(bnd_dict: dict, margin: float, is_inter: bool) -> list:
    ...
    for xmin, xmax, ymin, _ in bnd_dict['t']:
        vio_zone_list.append((
            xmin - margin,
            xmax + margin,
            ymin - (margin if is_inter else 0),
            ymin + (margin if is_outer else 0)
        ))
    ...
    for xmin, _, ymin, ymax in bnd_dict['l']:
        vio_zone_list.append((
            xmin - (margin if is_outer else 0),
            xmin + (margin if is_inter else 0),
            ymin - margin,
            ymax + margin
        ))
...
def _check_net_keep_out(test_net_list: list, def_data: dict, unit: float, 
...
def check_bnd_wire_out(out_fpath, config: dict, def_parser_dict: dict, 
    ...
    for blk, blk_data in config['block'].items():
        ...
        vio_zone_list = _get_vio_zone(def_data['diearea'], margin, True)

        # Net check by the test zone
        vio_net_list = _check_net_keep_out(blk_data['net'], def_data, config['unit'], 
...
def check_ma_wire_out(out_fpath, config: dict, macro_dict: dict, 
    ...
    for blk, blk_data in config['block'].items():
        ...
        for macro_info in blk_data['macro']:
            ...
            # Get the boundary violation zone
            vio_zone_list = _get_vio_zone(macro_coor, margin, False)
            print('\n', vio_zone_list, '\n')
