...
class TimePath:
    """Data container of a timing path."""
...
class TimeReport:
    ...
    def __init__(self, cell_ckp:set=None, inst_ckp:set=None, 
        ...
        ### attribute
        self._cell_ckp = set() if cell_ckp is None else dp(cell_ckp)
        self._inst_ckp = set() if inst_ckp is None else dp(inst_ckp)
        self._ickp_re = [] if ickp_re is None else dp(ickp_re)
        self._hcd = {} if hcd is None else dp(hcd)
        self._ckt = [] if ckt is None else dp(ckt)
        self._ckm = [] if ckm is None else dp(ckm)
        self._ckm_nock = ckm_nock
        self._dpc = dpc
        self._pc = {} if pc is None else dp(pc)
        ### data
    ...
    def parse_report(self, rpt_fp, prange:list=None):
        ...
        for range_ in prange:
            ...
            while True:
                ...
                if is_eof:
                    return
                elif p_ed is not None and fp_no >= p_ed:
                    break
    ...
    def clock_path_check(self, pid: int=0, is_dump: bool=False) -> tuple:
        ...
        if self._ckt:
            ...
            for pin in epath:
                ...
                if not ec_pass and self._ckm_nock:
                    ...
                    for ckm_re in ckm_list2:
                        ec_is_ckm = (True if ckm_re.fullmatch(pin[PT]) 
                                     else False)
                        if ec_is_ckm:
                            pvckm_re = ckm_re
                            break
        ...
        if is_dump:
            ...
            with open(f"clock_check{pid}.dump", "w") as f:
                ...
                scan_list = ([('Launch', sct_list, sc_col_sz)] if sct_list 
                             else [])
